...
# Đảm bảo bạn đã tải model đúng định dạng q4_k_m
MODEL_PATH = "models/qwen2.5-3b-instruct-q4_k_m.gguf" 

print("🔄 Đang nạp mô hình Qwen 2.5 3B vào VRAM GPU GTX 1650...")
llm = Llama(
    model_path=MODEL_PATH, 
    n_ctx=1024,          
    n_gpu_layers=-1,     # Tối đa hóa hiệu suất phần cứng
    verbose=False        
)
...
def process_chat_llm():
    ...
    try:
        ...
        full_prompt = (
            ...
            "3. Nếu câu hỏi hoàn toàn KHÔNG liên quan đến xe điện, lạc đề, hoặc là câu hỏi bẫy vô nghĩa, bạn PHẢI trả lời chính xác cụm từ: OUT_OF_SCOPE_TRIGGERED\n"
            "<|im_end|>\n"
            f"<|im_start|>user\n{user_message}<|im_end|>\n"
            "<|im_start|>assistant\n"
        )
        ...
        output = llm(
            full_prompt,
            max_tokens=150,      
            temperature=0.0,     # Bắt buộc bằng 0 để tuân thủ 100% Guardrails
            top_p=0.85,
        ...
        return jsonify({
            "status": "error", 
            "reply": "Dạ, hệ thống đang bận, anh/chị thử lại sau nhé!"
        }), 500
